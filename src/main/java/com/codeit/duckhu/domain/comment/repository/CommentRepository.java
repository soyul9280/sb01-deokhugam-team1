...
public interface CommentRepository extends JpaRepository<Comment, UUID>, CommentCustomRepository {
   ...
   */
  int countByReviewAndIsDeletedFalseAndCreatedAtBetween(
      Review review, Instant startTime, Instant endTime);

  /**
   * 특정 리뷰에 대해 모든 기간 동안 작성된 (삭제되지 않은) 댓글 수를 계산 (ALL TIME 기간 계산 시 사용)
   *
   * @param review 리뷰 엔티티
   * @return 전체 댓글 수
   */
