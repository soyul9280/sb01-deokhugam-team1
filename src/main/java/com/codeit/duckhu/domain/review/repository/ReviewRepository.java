...
public interface ReviewRepository extends JpaRepository<Review, UUID>, ReviewRepositoryCustom {
  ...
  @Query("SELECT COUNT(r) FROM Review r WHERE r.book.id = :bookId AND r.createdAt BETWEEN :from AND :to AND r.isDeleted = false")
  int countByBookIdAndCreatedAtBetween(@Param("bookId") UUID bookId, @Param("from") Instant from,
      @Param("to") Instant to);
