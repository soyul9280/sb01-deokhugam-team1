...
class BookControllerTest {
  ...
  class GetBook {
    ...
    void getBooks_shouldReturnBookList() throws Exception {
      // given
      CursorPageResponseBookDto fakeResponse = new CursorPageResponseBookDto(List.of(), null, null,
          0,
          0, false);
  ...
  class UpdateBook {
    ...
    void updateBook_shouldReturn200AndBookDto() throws Exception {
      ...
      BookUpdateRequest updateRequest = new BookUpdateRequest("수정된 제목", "수정된 작가", "수정된 설명", "수정된 출판사", LocalDate.now());

      MockMultipartFile bookDataPart = new MockMultipartFile("bookData", null, "application/json", objectMapper.writeValueAsBytes(updateRequest));
      MockMultipartFile thumbnailPart = new MockMultipartFile("thumbnailImage", "image.jpg", MediaType.IMAGE_JPEG_VALUE, "fake-thumbnail-content".getBytes());

      BookDto response = new BookDto(bookId, "수정된 제목", "수정된 작가", "수정된 설명", "수정된 출판사", LocalDate.now(), "1234567890", "https://s3.com/thumb.jpg", 3, 4.5, Instant.now(), Instant.now());
