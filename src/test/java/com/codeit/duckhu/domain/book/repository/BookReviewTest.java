...
public class BookReviewTest {
  ...
  void countAndAverageRating_withReviews() {
    ...
    Review r1 =
        Review.builder().book(book).user(user1).rating(4).content("좋아요").isDeleted(false).build();
    ...
    Book book2 =
        bookRepository.save(
            Book.builder()
                ...
                .author("저자2")
                .publisher("출판사")
                .publishedDate(LocalDate.of(2022, 1, 2))
                .isDeleted(false)
                .build());

    User user =
        userRepository.save(
            User.builder().email("user@test.com").nickname("user").password("1234").build());

    reviewRepository.save(
        Review.builder().book(book1).user(user).rating(4).content("좋아요").isDeleted(false).build());
    reviewRepository.save(
        Review.builder().book(book1).user(user).rating(5).content("굿").isDeleted(false).build());
    reviewRepository.save(
        Review.builder()
            .book(book2)
            .user(user)
  ...
  void averageRatingByBookIds_batchQuery() {
    ...
    reviewRepository.save(
        Review.builder().book(book1).user(user).rating(4).content("좋아요").isDeleted(false).build());
    reviewRepository.save(
        Review.builder().book(book1).user(user).rating(2).content("별로").isDeleted(false).build());
    reviewRepository.save(
        Review.builder()
            .book(book2)
            .user(user)
