...
class NotificationIntegrationTest {
  ...
  void listNotifications_unauthorized() throws Exception {
    mockMvc
        .perform(get("/api/notifications").param("userId", RECEIVER_ID.toString()))
  ...
  void markAllAsRead_success() throws Exception {
    mockMvc
        .perform(patch("/api/notifications/read-all").session(session))
        .andExpect(status().isNoContent());

    // 처리 후 조회하면 confirmed=true 로 바뀌었는지 확인
    mockMvc
        .perform(get("/api/notifications").session(session).param("userId", RECEIVER_ID.toString()))
        .andExpect(status().isOk())
